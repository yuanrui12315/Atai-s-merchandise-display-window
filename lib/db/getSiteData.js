 ...
 *
 */
/** 升级缓存键，避免旧版错误数据长期留在 Memory/Redis/File 中导致全站一直显示「无法获取 Notion 数据」 */
const SITE_DATA_CACHE_VERSION = 'v4'

/** Vercel / 本地日志检索：固定前缀 + JSON，便于对照「占位」究竟走哪条分支（非猜测） */
function maskPageIdForLog(pageId) {
  if (pageId == null || pageId === '') return ''
  const s = String(pageId).replace(/-/g, '')
  return s.length <= 10 ? s : `${s.slice(0, 4)}…${s.slice(-4)}`
}

function logSiteDataOutcome(reason, pageId, extra = {}) {
  try {
    console.error(
      '[NOTION_SITE_DATA]',
      JSON.stringify({ reason, pageId: maskPageIdForLog(pageId), ...extra })
    )
  } catch (_) {
    /* ignore */
  }
...
export async function getGlobalData({
  ...
  const siteIds = (pageId && String(pageId).split(',')) || []
  const siteIdsTrimmed = siteIds.map(s => String(s).trim()).filter(Boolean)
  let data = EmptyData(pageId)

  if (!siteIdsTrimmed.length) {
    const msg =
      '[getGlobalData] NOTION_PAGE_ID 为空或未配置，请检查本地 .env 与 Vercel Environment Variables（Production / Preview 是否勾选）'
    console.error(msg)
    logSiteDataOutcome('NOTION_PAGE_ID_EMPTY', pageId, { from })
    // 构建期直接失败，避免带着占位数据部署上线
  ...
  try {
    for (let index = 0; index < siteIdsTrimmed.length; index++) {
      const siteId = siteIdsTrimmed[index]
    ...
    logSiteDataOutcome('GET_GLOBAL_DATA_THROW', pageId, {
      from,
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  return await getOrSetDataWithCache(
    `site_data_${SITE_DATA_CACHE_VERSION}_${pid}`,
    async (id, fromArg) => {
      const pageRecordMap = await getPage(id, fromArg)
      return convertNotionToSiteData(id, fromArg, deepClone(pageRecordMap))
    },
    pid,
    from
  )
...
const EmptyData = pageId => {
  const empty = {
    ...
    allPages: [
      {
        // 占位 id 必须为字符串，否则 getPostBlocks 会传数字给 notion-utils 导致 e.split is not a function
...
/**
 * recordMap.block[id] 常为 { role, value }；部分接口会套一层 { value: { value: 真实块 } }，
 * 直接读 .value 会没有 .type，误判「不是数据库」→ 全站 EmptyData。
 * 与 notion-utils getBlockValue / 本站 unwrapRecordMapBlockValueLayers 语义对齐。
 */
function getRecordMapEntryBlockValue(entry) {
...
/** block 的 key 与 pageId 可能仅大小写/横线不一致，需全表扫描一次 */
function findRecordMapBlockEntryForPageId(block, pageUuid, pageUuidCompact) {
  if (!block || typeof block !== 'object') return undefined
  const direct = block[pageUuid] || block[pageUuidCompact]
  if (direct) return direct
  const target = String(pageUuidCompact || pageUuid.replace(/-/g, '')).toLowerCase()
  if (!target) return undefined
  for (const k of Object.keys(block)) {
    if (String(k).replace(/-/g, '').toLowerCase() === target) {
      return block[k]
    }
  }
  return undefined
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  if (!pageRecordMap) {
    ...
    logSiteDataOutcome('NO_PAGE_RECORDMAP', pageId, { from })
    return {}
  }
  pageId = idToUuid(pageId)
  let block = pageRecordMap.block || {}
  // Notion API 可能用带横线或不带横线的 ID 作为 block 的 key，两种都尝试
  const blockIdNoDashes = pageId.replace(/-/g, '')
  const rootBlock = findRecordMapBlockEntryForPageId(
    block,
    pageId,
    blockIdNoDashes
  )
  const rawMetadata = getRecordMapEntryBlockValue(rootBlock)
  // 若仍找不到，尝试从 block 中查找类型为数据库的根块（兼容部分 Notion 返回结构）
  let metadata = rawMetadata
  if (!metadata && Object.keys(block).length > 0) {
    const dbBlock = Object.values(block).find(b => {
      const bv = getRecordMapEntryBlockValue(b)
      return (
        bv &&
        (bv.type === 'collection_view_page' || bv.type === 'collection_view')
      )
    })
    metadata = getRecordMapEntryBlockValue(dbBlock)
  }
  // 当根块是普通 page 时，可能是「页面内嵌数据库」：根块 type=page，子块中有 collection_view
  // notion-client 文档：第一个 id 是 collection view，第二个是父页面
  if (!metadata?.collection_id && rawMetadata?.type === 'page' && rawMetadata?.content?.length > 0) {
    for (const childId of rawMetadata.content) {
      const childIdNorm = childId.replace(/-/g, '')
      const child = findRecordMapBlockEntryForPageId(block, childId, childIdNorm)
      const childVal = getRecordMapEntryBlockValue(child)
      if (
        childVal?.collection_id &&
        (childVal.type === 'collection_view' ||
          childVal.type === 'collection_view_page')
      ) {
        metadata = childVal
        break
      }
  ...
  // 若 block 中仍无有效 metadata，但存在 collection/collection_view，则从 collection_view 或 collection 提取
  if (!metadata?.collection_id && (pageRecordMap.collection_view || pageRecordMap.collection)) {
    const firstView = pageRecordMap.collection_view && Object.values(pageRecordMap.collection_view)[0]
    const collectionId = firstView?.value?.collection_id || (pageRecordMap.collection && Object.keys(pageRecordMap.collection)[0])
    if (collectionId) {
      metadata = {
        type: 'collection_view',
        collection_id: collectionId,
        view_ids: firstView?.value?.view_ids || []
      }
    }
  }
  // 判定是否可当作商品数据库：有 collection_id 即可（部分 Notion 返回 type 字段异常/新枚举）
  let canUseDatabase =
  ...
  if (!canUseDatabase) {
    for (const k of Object.keys(block)) {
      const bv = getRecordMapEntryBlockValue(block[k])
      if (bv?.collection_id) {
        metadata = bv
        canUseDatabase = true
        break
  ...
  if (!canUseDatabase) {
    const blockTypes = Object.values(block)
      .map(b => getRecordMapEntryBlockValue(b)?.type || b?.value?.type)
      .filter(Boolean)
    console.error(
      `pageId "${pageId}" is not a database. ` +
      `Block keys: [${Object.keys(block).slice(0, 3).join(', ')}...], ` +
      `types found: [${blockTypes.slice(0, 5).join(', ')}]. ` +
      `请确认：1) 使用整页数据库（新建页面后输入 /gallery 创建） 2) 集成已连接该页`
    )
    logSiteDataOutcome('NOT_DATABASE', pageId, {
      from,
      blockKeyCount: Object.keys(block).length,
      typesSample: blockTypes.slice(0, 8)
    })
    return EmptyData(pageId)
  }
  const collectionId = metadata?.collection_id
  // 兼容 notion-client 返回的嵌套结构：{ value: { value: { schema, ... }, role } }
  const unwrap = (o) => o?.value?.value ?? o?.value ?? o
  const collection = unwrap(collectionId && pageRecordMap.collection?.[collectionId]) ||
    unwrap(Object.values(pageRecordMap.collection || {})[0]) || {}
  ...
  if (pageIds?.length === 0) {
    // 当 getPage 未正确拉取 collection_query 时，尝试直接用 getCollectionData 拉取数据库
    if (collectionId && (viewIds?.length > 0 || BLOG.NOTION_VIEW_ID)) {
      try {
        const groupIndex = BLOG.NOTION_INDEX || 0
        // 优先使用 NOTION_VIEW_ID（从 Notion URL 的 v= 参数获取），否则用 viewIds
        const viewId = BLOG.NOTION_VIEW_ID || viewIds?.[groupIndex] || viewIds?.[0]
        const collectionViewValue = collectionView?.[viewId]?.value || {}
        const collResult = await notionAPI.getCollectionData(
          collectionId,
          viewId,
          collectionViewValue,
          { limit: 9999 }
        )
        const reducerResults = collResult?.result?.reducerResults ?? collResult?.reducerResults
        if (reducerResults) {
          pageRecordMap.collection_query = pageRecordMap.collection_query || {}
          pageRecordMap.collection_query[collectionId] = pageRecordMap.collection_query[collectionId] || {}
          pageRecordMap.collection_query[collectionId][viewId] = reducerResults
          if (collResult.recordMap?.block) {
            pageRecordMap.block = { ...pageRecordMap.block, ...collResult.recordMap.block }
            block = pageRecordMap.block
          }
          if (collResult.recordMap?.collection) {
            pageRecordMap.collection = { ...pageRecordMap.collection, ...collResult.recordMap.collection }
          }
          const effectiveViewIds = viewIds?.length ? viewIds : [viewId]
        ...
        console.warn('[getCollectionData 备用拉取失败]', e?.message || e)
      }
    }
    if (pageIds?.length === 0) {
      console.error(
      ...
      logSiteDataOutcome('EMPTY_ARTICLE_LIST', pageId, { from })
      return EmptyData(pageId)
    }
  }

  // 抓取主数据库最多抓取1000个blocks，溢出的数block这里统一抓取一遍
  // 注意：notion-client/syncRecordValues 返回的 block 结构可能是 { spaceId, value: { value, role } }，实际数据在 .value.value
  // 若 collection 返回的 block 缺少 format.page_cover，则通过 syncRecordValues 重新拉取完整 block（含封面）
  const blockIdsNeedFetch = []
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    const b = block[id]
    const value = b?.value?.value ?? b?.value
    const needFetch = !value || !value.properties || !value?.format?.page_cover
    if (needFetch) {
  ...
  // 规范化 block 结构：syncRecordValues 返回 { value: { value, role } }，实际数据在 .value.value
  for (const bid of Object.keys(block)) {
    const b = block[bid]
    if (b?.value?.value && !b?.value?.properties) {
      block[bid] = { ...b, value: b.value.value }
    }
  }

  // 构建 recordMap 供 getPageProperty 使用（notion-utils 用 o.collection[block.parent_id] 查找 schema）
  const collections = pageRecordMap.collection || {}
  const collectionMap = { ...collections }
  const dbRootBlock = block[pageId] || block[pageId?.replace(/-/g, '')]
  const rootCollectionId = dbRootBlock?.value?.collection_id || collectionId
  const collEntry = collectionMap[rootCollectionId] || Object.values(collectionMap)[0]
  if (collEntry) {
    const idsToAdd = new Set([pageId, pageId?.replace(/-/g, ''), rootCollectionId])
    pageIds.forEach(pid => {
      const v = block[pid]?.value || block[pid?.replace(/-/g, '')]?.value
      if (v?.parent_id) idsToAdd.add(v.parent_id)
    })
    idsToAdd.forEach(pid => { if (pid && !collectionMap[pid]) collectionMap[pid] = collEntry })
  }
  const recordMap = {
    block,
    collection: collectionMap,
    collection_view: pageRecordMap.collection_view || {}
  }
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    const blockObj = block[id] || fetchedBlocks[id] || { value }
    const properties =
  ...
  const latestPosts = getLatestPosts({ allPages, from, latestPostCount: 12 })
...
export function getNavPages({ allPages }) {
  ...
  return allNavPages.map(item => ({
    ...
    ext: item.ext || {},
    heroPinOrder: item.heroPinOrder || 0
  }))
}