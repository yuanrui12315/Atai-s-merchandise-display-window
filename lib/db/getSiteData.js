...
const SITE_DATA_CACHE_VERSION = 'v3'
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  const rootBlock = block[pageId] || block[blockIdNoDashes]
  ...
  if (!metadata?.collection_id && rawMetadata?.type === 'page' && rawMetadata?.content?.length > 0) {
    for (const childId of rawMetadata.content) {
      ...
      const child = block[childId] || block[childIdNorm] || block[childId.replace(/-/g, '')]
      const childVal = getRecordMapEntryBlockValue(child)
      if (childVal?.type === 'collection_view' && childVal.collection_id) {
        metadata = childVal
        break
      }
    }
  ...
  if (
    ...
    const blockTypes = Object.values(block).map(b => b?.value?.type).filter(Boolean)
    console.error(
