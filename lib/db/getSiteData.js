...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // 优先使用与 collectionId 匹配的 collection，确保 schema 正确
  const collection = (collectionId && pageRecordMap.collection?.[collectionId]?.value) ||
    Object.values(pageRecordMap.collection || {})[0]?.value || {}
