export default {
  LOCALE: '中文(简体)',
  MENU: {
    WALK_AROUND: '随便逛逛',
    CATEGORY: '更多分类',
    TAGS: '商品标签',
    SHARE_URL: '分享地址',
    DARK_MODE: '深色模式',
    LIGHT_MODE: '浅色模式',
    THEME_SWITCH: '主题切换',
    COPY: '复制'
  },
  NAV: {
    INDEX: '首页',
    BACK_HOME: '回到首页',
    RSS: '订阅',
    SEARCH: '搜索商品',
    ABOUT: '关于',
    NAVIGATOR: '导航',
    MAIL: '邮箱',
    ARCHIVE: '归档',
    PAGE_NOT_FOUND: '页面找不到啦',
    PAGE_NOT_FOUND_REDIRECT: '页面无法加载，即将返回首页'
  },
  COMMON: {
    THEME: 'Theme',
    SIGN_IN: '登录',
    SIGN_OUT: '登出',
    ARTICLE_LIST: '文章列表',
    RECOMMEND_POSTS: '推荐文章',
    MORE: '更多',
    NO_MORE: '没有更多了',
    SEARCH_NO_RESULTS: '未找到相关商品',
    SEARCH_NO_RESULTS_HINT: '请检查关键词是否正确，或换个词试试。',
    /** 搜索无结果时整页空状态（heo BlogPostListEmpty） */
    SEARCH_LIST_EMPTY:
      '没有找到您想要的商品哦！也有可能未上架..可直接微信咨询！',
    LATEST_POSTS: '最新发布',
    TAGS: '标签',
    NO_TAG: 'NoTag',
    CATEGORY: '分类',
    SHARE: '分享',
    SCAN_QR_CODE: '微信扫码分享',
    URL_COPIED: '链接已复制！',
    TABLE_OF_CONTENTS: '目录',
    RELATE_POSTS: '相关商品',
    COPYRIGHT: '声明',
    AUTHOR: '作者',
    URL: '链接',
    ANALYTICS: '统计',
    RECOMMEND_BADGES: '荐',
    BLOG: '博客',
    NOW: '回归',
    POSTS: '篇文章',
    ARTICLE: '文章',
    VISITORS: '位访客',
    VIEWS: '次查看',
    PAGE_URL_COPIED: '页面地址已复制',
    COPYRIGHT_NOTICE: '本文采用 CC BY-NC-SA 4.0 许可协议，转载请注明出处。',
    RESULT_OF_SEARCH: '篇搜索到的结果',
    NO_RESULTS_FOUND: '没有找到商品',
    ARTICLE_DETAIL: '文章详情',
    PASSWORD_ERROR: '密码错误！',
    ARTICLE_LOCK_TIPS: '文章已上锁，请输入访问密码',
    ARTICLE_UNLOCK_TIPS: '文章已解锁',
    SUBMIT: '提交',
    POST_TIME: '发布于',
    LAST_EDITED_TIME: '最后更新',
    COMMENTS: '评论',
    RECENT_COMMENTS: '最新评论',
    DEBUG_OPEN: '开启调试',
    DEBUG_CLOSE: '关闭调试',
    THEME_SWITCH: '切换主题',
    ANNOUNCEMENT: '公告',
    START_READING: '开始阅读',
    MINUTE: '分钟',
    WORD_COUNT: '字数',
    READ_TIME: '阅读时长',
    NEXT_POST: '下一篇',
    PREV_POST: '上一篇',
    NOT_FOUND: '页面未找到'
  },
  PAGINATION: {
    PREV: '上一页',
    NEXT: '下一页',
    FIRST: '首页',
    LAST: '尾页',
    RANGE: '显示第 %start%-%end% 条，共 %total% 件商品'
  },
  SEARCH: {
    ARTICLES: '搜索商品',
    TAGS: '搜索商品'
  },
  POST: {
    BACK: '返回上页',
    TOP: '回到顶部'
  },
  MAILCHIMP: {
    SUBSCRIBE: '邮件订阅',
    MSG: '订阅以获取每月更新的新闻和文章，直接发送至您的邮箱。',
    EMAIL: '邮箱'
  },
  AI_SUMMARY: {
    NAME: 'AI智能摘要'
  }
}
