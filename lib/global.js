...
export function GlobalContextProvider(props) {
  ...
  //
  // /search/*：客户端 router.push 会等到该页 getStaticProps（含全文检索）结束才结束导航，期间可能长时间
  // 不触发 routeChangeComplete → 全屏 LoadingCover 会一直卡在 95%。此处不对此类路径打开 LoadingCover。
  useEffect(() => {
    let loadingCapTimer = null

    const clearLoadingCap = () => {
      if (loadingCapTimer != null) {
    ...
    const handleStart = url => {
      ...
      const isSearchRoute =
        pathOnly === '/search' || pathOnly.startsWith('/search/')
      if (isSearchRoute) {
        return
      }
