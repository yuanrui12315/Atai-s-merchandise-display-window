...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  const nested = raw.value && typeof raw.value === 'object' ? raw.value : {}
  const blockIdStr = String(
    raw.id != null && raw.id !== ''
      ? raw.id
      : nested.id != null && nested.id !== ''
        ? nested.id
        : ''
  ).trim()
  const b = {
    ...nested,
    ...raw,
    id: blockIdStr || raw.id || nested.id,
    type: raw.type || nested.type
  }
  ...
  // Notion旧图床（id 缺失时勿拼 undefined，否则全站图会挂）
  if (needConvert && blockIdStr) {
    ret =
      BLOG.NOTION_HOST +
      '/image/' +
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    ...
    if (
      ...
      ret = `${ret.trim()}${separator}t=${blockIdStr}`
    }
  }

  // 统一压缩图片
  if (needCompress) {
    const width = b?.format?.block_width
    ret = compressImage(ret, width)
  }

  // 图片代理：解决国内访问 Notion 图片慢的问题，通过本站中转
  if (ret && BLOG.IMAGE_PROXY_ENABLE && isNotionImageUrl(ret)) {
    const baseUrl = BLOG.LINK || (typeof window !== 'undefined' ? window.location.origin : '')
    if (baseUrl) {
      ret = `${baseUrl}/api/proxy-image?url=${encodeURIComponent(ret)}`
    }
  }
...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  // Notion /image/ 代理接口、secure.notion-static.com 等
  if (
    (image.indexOf(BLOG.NOTION_HOST) === 0 && image.includes('/image/')) ||
    image.includes('secure.notion-static.com') ||
    image.includes('prod-files-secure')
  ) {
    params.set('width', width)
    params.set('cache', 'v2')
    urlObj.search = params.toString()
