...
async function rewriteSyncEndpoint(options) {
  ...
  if (raw == null) return
  // got 在 _makeRequest 里用 url.protocol；必须是 URL 实例，不能写成普通字符串（会触发 ERR_INVALID_PROTOCOL）
  const href = typeof raw === 'string' ? raw : raw.toString()
  if (
    !href.includes('/api/v3/syncRecordValues') ||
    href.includes('syncRecordValuesMain')
  ) {
    return
  }
 ...
 * 对 4xx/5xx（含 429），got 会先跑 afterResponse 再跑 beforeError；若两处各 shift 一次队列会错位，
 * 多个请求会同时「放行」→ 并发打爆 Notion。此处同一请求只释放一次（幂等）。
 */
function createGotThrottleMerge() {
  let tail = Promise.resolve()
  /** @type {null | (() => void)} */
  let activeRelease = null
  const gapMs = Number(process.env.NOTION_HTTP_GAP_MS || 650)

  const finishHttpSlot = () => {
    if (!activeRelease) return
    const fn = activeRelease
    activeRelease = null
    fn()
  }
  ...
  return function mergeGotThrottleOptions(userGot = {}) {
    ...
    const throttleBeforeError = async (error) => {
      finishHttpSlot()
      return error
    }
...
export const notionAPI = {
  // notion 内部 p-map 默认 concurrency=3，易叠请求；构建期与 got 串行配合，强制 1
  getPage: (id, maybeOpts) =>
    callNotion('getPage', id, {
      concurrency: 1,
