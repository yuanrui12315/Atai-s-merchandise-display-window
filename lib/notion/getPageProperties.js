...
import { getBlockTitle, getDateValue, getTextContent } from 'notion-utils'
...
export default async function getPageProperties(
  ...
  // 1. 优先用 notion-utils 的 getBlockTitle 提取标题
  const blockTitle = getBlockTitle(value, null)
  if (blockTitle && String(blockTitle).trim()) {
    properties.title = blockTitle
  }
  // 2. 若 value.properties 中有 key 为 title 的列，也尝试提取
  ...
  // 3. 从 schema 中查找 type===title 的属性（Notion 数据库标题列）
  if ((!properties.title || !properties.title.trim()) && schema && typeof schema === 'object') {
    for (const [propId, propSchema] of Object.entries(schema)) {
      if (propSchema?.type === 'title' && value?.properties?.[propId]) {
        const t = getTextContent(value.properties[propId])
        if (t && String(t).trim()) {
          properties.title = t
          break
        }
...
properties.allCategories = rawCate || ''
  ...
  // 标题兜底：Notion 默认 Name，常见列名：Aa title/标题/商品名称，以及 NOTION_PROPERTY_NAME.title 配置
