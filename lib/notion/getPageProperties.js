...
export default async function getPageProperties(
  ...
  const getPropVal = (propId) => {
    const props = value?.properties || {}
    return props[propId] ?? props[normId(propId)] ?? (() => { const k = Object.keys(props).find(x => normId(x) === normId(propId)); return k ? props[k] : undefined })()
  }

  // 优先按 schema 遍历提取（兼容 value.properties 的 key 与 schema key 格式不一致）
  if (schema && typeof schema === 'object') {
    for (const [propId, propSchema] of Object.entries(schema)) {
      const rawVal = getPropVal(propId)
      if (!rawVal || !propSchema?.name) continue
      if (propSchema.type === 'title') {
        const t = getTextContent(rawVal)
        if (t && String(t).trim()) properties[propSchema.name] = t
      } else if (propSchema.type === 'number') {
        let numVal = rawVal?.[0]?.[1] ?? rawVal?.[0]?.[0]
        if (numVal === undefined && Array.isArray(rawVal?.[0])) numVal = rawVal[0][rawVal[0].length - 1]
        const parsed = typeof numVal === 'number' ? numVal : parseFloat(String(numVal || getTextContent(rawVal) || ''))
        if (Number.isFinite(parsed)) properties[propSchema.name] = parsed
      } else if (propSchema.type === 'date') {
        const dateProperty = getDateValue(rawVal)
        if (dateProperty) { delete dateProperty?.type; properties[propSchema.name] = dateProperty }
      } else if (propSchema.type === 'select' || propSchema.type === 'multi_select') {
        const selects = getTextContent(rawVal)
        if (selects && String(selects).trim()) properties[propSchema.name] = selects.split(',').map(s => s?.trim()).filter(Boolean)
      } else if (!['person', 'date', 'select', 'multi_select'].includes(propSchema.type)) {
        const t = getTextContent(rawVal)
        if (t !== undefined && t !== null) properties[propSchema.name] = t
      }
    }
  }
  ...
  const SCHEMA_NAME_ALTERNATIVES = {
    ...
    slug: ['slug', '自定义链接', '链接别名', fieldNames.slug].filter(Boolean),
...
properties.allCategories = rawCate || ''
  ...
  // mapImgUrl 需要 block.id 构建 Notion 图片代理 URL，确保 id 正确传递
  const blockId = blockObj?.value?.id ?? value?.id ?? id
  const blockForImg = { ...(blockObj || {}), id: blockId }
  ...
  let pageCoverThumbnail =
    mapImgUrl(value?.format?.page_cover, blockForImg, 'block') ?? ''
  const listCoverCap = Number(BLOG.POST_LIST_COVER_MAX_WIDTH)
  if (
    pageCoverThumbnail &&
    Number.isFinite(listCoverCap) &&
    listCoverCap >= 32
  ) {
    pageCoverThumbnail =
      applyWidthCapToImageSrc(pageCoverThumbnail, listCoverCap) ||
      pageCoverThumbnail
  }
  ...
  // 首页 Hero 置顶：读取 Notion 数字列（列名可配 NOTION_PROPERTY_NAME.hero_pin，默认「首页置顶」）
  const heroPinProp = fieldNames.hero_pin || '首页置顶'
  const heroPinKeys = [...new Set([heroPinProp, '首页置顶', 'Hero置顶', 'hero_pin'].filter(Boolean))]
  for (const key of heroPinKeys) {
    const v = properties[key]
    if (v === undefined || v === null || v === '') continue
    const n = typeof v === 'number' ? v : parseFloat(String(v).replace(/,/g, ''))
    if (Number.isFinite(n) && n > 0) {
      properties.heroPinOrder = n
      break
    }
  }

  stripPrivateNotionPropertiesFromPost(properties)

  return properties
}

/**
 * 移除 Notion「仅自己看」的列，避免进入 allPages / getStaticProps（见 conf/notion.config.js NOTION_PRIVATE_PROPERTY_NAMES）
 */
function stripPrivateNotionPropertiesFromPost(properties) {
  const raw = BLOG.NOTION_PRIVATE_PROPERTY_NAMES
  if (!raw || !properties || typeof properties !== 'object') return
