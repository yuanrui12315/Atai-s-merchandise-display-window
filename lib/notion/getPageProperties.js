...
// 1. 获取原始分类数据
  const rawCate = properties.category
  
  // 2. 统一转为“纯文字数组”，这样点击跳转才有内容，且能显示多个
  if (Array.isArray(rawCate)) {
    // 兼容 Notion API 传回的对象数组或字符串数组
    properties.category = rawCate.map(c => {
      if (typeof c === 'object') return c.name || c.select?.name || ''
      return String(c)
    }).filter(Boolean)
  } else if (rawCate && typeof rawCate === 'object') {
    // 兼容单选属性
    properties.category = [rawCate.name || rawCate.select?.name || '']
  } else if (typeof rawCate === 'string' && rawCate.includes(',')) {
    // 兼容带逗号的字符串
    properties.category = rawCate.split(',').map(c => c.trim())
  } else {
    // 兜底：如果是单个字符串或空值
    properties.category = rawCate ? [String(rawCate)] : []
  }
