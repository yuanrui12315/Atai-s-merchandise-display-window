...
/**
 * 正文的顶层子块 id 列表。列表页/全站 `post` 在 getPageProperties 末尾会 `delete post.content` 以瘦身，
 * 但 `pageBlockMap.block[页id].value.content` 仍有；搜索与 Algolia 等依赖纯文本时须从此回退。
 * @param {Object} post
 * @param {Object} pageBlockMap
 * @returns {string[]}
 */
function getPageRootContentIds(post, pageBlockMap) {
  if (post?.content?.length) return post.content
  if (!pageBlockMap?.block || !post?.id) return []
  const blocks = pageBlockMap.block
  const pid = post.id
  if (blocks[pid]?.value?.content?.length) {
    return blocks[pid].value.content
  }
  const rootTypes = new Set(['page', 'collection_view_page'])
  for (const key of Object.keys(blocks)) {
    const v = blocks[key]?.value
    if (
      v &&
      notionIdsEqual(v.id, pid) &&
      rootTypes.has(v.type) &&
      Array.isArray(v.content)
    ) {
      return v.content
    }
  }
  // 与页 id 相同且带 content 的任意根块（兼容未收录的 type）
  for (const key of Object.keys(blocks)) {
    const v = blocks[key]?.value
    if (
      v &&
...
export function getPageContentText(post, pageBlockMap) {
  ...
  const postContent = getPageRootContentIds(post, pageBlockMap)
  let contentTextList = []
  // 防止搜到加密文章的内容
