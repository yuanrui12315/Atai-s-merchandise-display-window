...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  // 不蒜子外网统计已全局关闭（避免国内超时/拖慢）；Notion 配置表也不可再打开
  if (key === 'ANALYTICS_BUSUANZI_ENABLE') {
    return false
  }
  ...
  switch (key) {
    case 'HOME_LIST_MOBILE_PROXY_QUALITY':
      // 以主题 extendConfig 为准，避免 Notion 表里误填 0 关闭手机列表 WebP 画质
      return convertVal(
    ...
    case 'UUID_REDIRECT':
      ...
      // POSTS_PER_PAGE：blog.config.js 优先，避免 Notion 配置表覆盖
      if (key === 'POSTS_PER_PAGE' && hasVal(BLOG[key])) {
        return convertVal(BLOG[key])
      }
      return convertVal(
        getValue(extendConfig[key], getValue(defaultVal, BLOG[key]))
      )
