 ...
 * - **SEARCH_BODY_BUDGET_MS**（默认 55000）：正文阶段最长毫秒，超出则停止拉正文（元数据已命中的仍会返回）。
 * - **SEARCH_BODY_MAX_POSTS**：正文候选**条数上限**（仅当环境变量设为**正整数**时生效）。**未设置**时不在条数上硬性截断，会在预算内尽量扫完所有「仅正文可能命中」的商品，便于正文里才有的词也能搜到；若遇 524 / 源站超时，再在 Vercel 设较小正数限流。
 *
...
export async function filterPostsByKeyword(allPosts, keyword, options = {}) {
  const filterPosts = []
  const k = searchKey(keyword)
  if (!k) return allPosts

  const envBudget = parseInt(process.env.SEARCH_BODY_BUDGET_MS || '', 10)
  const envMaxPostsRaw = process.env.SEARCH_BODY_MAX_POSTS
  const envMaxPosts =
    envMaxPostsRaw === undefined || envMaxPostsRaw === ''
      ? NaN
      : parseInt(String(envMaxPostsRaw), 10)
  ...
  for (const post of allPosts) {
    ...
    const metaHit = searchKey(articleInfo).indexOf(k) > -1
    post.results = []
    if (metaHit) {
...
async function tryBodyMatch(post, k) {
  ...
  if (
    ...
    searchKey(contentText).indexOf(k) > -1
  ) {
