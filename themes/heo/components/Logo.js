  ...
  return (
    <SmartLink
      ...
      className='flex h-full w-full min-w-0 max-w-full cursor-pointer flex-nowrap items-center gap-1 font-extrabold no-underline text-inherit sm:gap-1.5 lg:w-auto lg:gap-2'>
        ...
        <div className='order-2 flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded lg:order-1 lg:mr-4 lg:h-7 lg:w-7'>
          <LazyImage
            src={siteInfo?.icon}
            width={24}
            height={24}
            alt={siteConfig('AUTHOR')}
            className='h-7 w-7 rounded object-cover lg:h-7 lg:w-7'
          />
        </div>
        <span className='order-3 inline-flex shrink-0 rounded-lg border-2 border-current border-opacity-45 px-2 py-1 text-xs font-semibold transition-all duration-200 hover:border-opacity-85 hover:bg-indigo-500 hover:bg-opacity-10 sm:text-sm lg:ml-1 lg:border lg:border-opacity-40 lg:px-3 lg:py-0.5 lg:text-sm lg:font-medium dark:hover:bg-yellow-500 dark:hover:bg-opacity-10'>
          首页
        </span>
    </SmartLink>
  )
